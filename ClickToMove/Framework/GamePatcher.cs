...
namespace Raquellcesar.Stardew.ClickToMove.Framework
{
    ...
    public static class GamePatcher
    {
        ...
        /// <summary>
        ///     The mouse left button state on the last tick. This is the real state of the button,
        ///     regardless of it being suppressed by any mod.
        /// </summary>
        ...
        public static void Hook(HarmonyInstance harmony)
        {
            ...
            harmony.Patch(
                ...
                prefix: new HarmonyMethod(typeof(GamePatcher), nameof(GamePatcher.BeforeTryToCheckAt)));
        ...
        ///     cref="ClickToMoveManager.JustClosedActiveMenu"/> so the current click and subsequent
        ...
        private static bool BeforeGameLocationCheckAction(GameLocation __instance, Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who, ref bool __result)
        {
            if (who.IsSitting())
            {
                who.StopSitting();
            ...
            foreach (Farmer current in __instance.farmers)
            {
                if (current != Game1.player && current.GetBoundingBox().Intersects(value) && current.checkAction(who, __instance))
                {
                    __result = true;
                    return false;
                }
            ...
            if (__instance.currentEvent != null && __instance.currentEvent.isFestival)
            {
                __result = __instance.currentEvent.checkAction(tileLocation, viewport, who);
                return false;
            }

            foreach (NPC current2 in __instance.characters)
            {
                if (current2 != null && !current2.IsMonster && (!who.isRidingHorse() || !(current2 is Horse)) && current2.GetBoundingBox().Intersects(value) && current2.checkAction(who, __instance))
                {
            ...
            if (who.IsLocalPlayer && who.currentUpgrade != null && __instance.name.Equals("Farm") && tileLocation.Equals(new Location((int)(who.currentUpgrade.positionOfCarpenter.X + 32f) / 64, (int)(who.currentUpgrade.positionOfCarpenter.Y + 32f) / 64)))
            {
            ...
            foreach (ResourceClump current3 in __instance.resourceClumps)
            {
                if (current3.getBoundingBox(current3.tile).Intersects(value) && current3.performUseAction(new Vector2(tileLocation.X, tileLocation.Y), __instance))
                {
                    __result = true;
                    return false;
                }
            ...
            if (__instance.objects.ContainsKey(vector) && __instance.objects[vector].Type != null)
            {
                if (who.isRidingHorse() && !(__instance.objects[vector] is Fence))
                {
                    __result = false;
                    return false;
                }

                if (vector.Equals(who.getTileLocation()) && !__instance.objects[vector].isPassable())
                {
                    Tool tool = new Pickaxe();
                    tool.DoFunction(Game1.currentLocation, -1, -1, 0, who);
                    if (__instance.objects[vector].performToolAction(tool, __instance))
                    {
                        __instance.objects[vector].performRemoveAction(__instance.objects[vector].tileLocation, Game1.currentLocation);
                        __instance.objects[vector].dropItem(__instance, who.GetToolLocation(), new Vector2(who.GetBoundingBox().Center.X, who.GetBoundingBox().Center.Y));
                    ...
                    tool = new Axe();
                    tool.DoFunction(Game1.currentLocation, -1, -1, 0, who);
                    if (__instance.objects.ContainsKey(vector) && __instance.objects[vector].performToolAction(tool, __instance))
                    {
                        __instance.objects[vector].performRemoveAction(__instance.objects[vector].tileLocation, Game1.currentLocation);
                        __instance.objects[vector].dropItem(__instance, who.GetToolLocation(), new Vector2(who.GetBoundingBox().Center.X, who.GetBoundingBox().Center.Y));
                        Game1.currentLocation.Objects.Remove(vector);
                    ...
                    if (!__instance.objects.ContainsKey(vector))
                    {
                        __result = true;
                        return false;
                    }
                ...
                if (__instance.objects.ContainsKey(vector) && (__instance.objects[vector].Type.Equals("Crafting") || __instance.objects[vector].Type.Equals("interactive")))
                {
                    if (who.ActiveObject == null && __instance.objects[vector].checkForAction(who))
                    {
                        __result = true;
                        return false;
                    }
                    ...
                    if (__instance.objects.ContainsKey(vector))
                    {
                        ...
                        {
                            SObject value2 = __instance.objects[vector].heldObject.Value;
                            __instance.objects[vector].heldObject.Value = null;
                            bool flag = __instance.objects[vector].performObjectDropInAction(who.CurrentItem, probe: true, who);
                            __instance.objects[vector].heldObject.Value = value2;
                            bool flag2 = __instance.objects[vector].performObjectDropInAction(who.CurrentItem, probe: false, who);
                            ...
                            __result = __instance.objects[vector].checkForAction(who) || flag;
                            return false;
                        }

                        __result = __instance.objects[vector].checkForAction(who);
                        return false;
                    }
                }
                else if (__instance.objects.ContainsKey(vector) && (bool)__instance.objects[vector].isSpawnedObject)
                {
                    int quality = __instance.objects[vector].quality;
                    Random random = new Random((int)Game1.uniqueIDForThisGame / 2 + (int)Game1.stats.DaysPlayed + (int)vector.X + (int)vector.Y * 777);
                    if (who.professions.Contains(16) && __instance.objects[vector].isForage(__instance))
                    {
                        __instance.objects[vector].Quality = 4;
                    }
                    else if (__instance.objects[vector].isForage(__instance))
                    {
                        if (random.NextDouble() < (double)((float)who.ForagingLevel / 30f))
                        {
                            __instance.objects[vector].Quality = 2;
                        }
                        else if (random.NextDouble() < (double)((float)who.ForagingLevel / 15f))
                        {
                            __instance.objects[vector].Quality = 1;
                        }
                    }

                    if (__instance.objects[vector].questItem.Value && __instance.objects[vector].questId.Value != 0 && !who.hasQuest(__instance.objects[vector].questId))
                    {
                        __result = false;
                        return false;
                    }

                    if (who.couldInventoryAcceptThisItem(__instance.objects[vector]))
                    {
                        ...
                        if (!__instance.isFarmBuildingInterior())
                        {
                            if (__instance.objects[vector].isForage(__instance))
                            {
                        ...
                        who.addItemToInventoryBool(__instance.objects[vector].getOne());
                        Game1.stats.ItemsForaged++;
                        if (who.professions.Contains(13)
                            && random.NextDouble() < 0.2
                            && !__instance.objects[vector].questItem
                            && who.couldInventoryAcceptThisItem(__instance.objects[vector])
                            && !__instance.isFarmBuildingInterior())
                        {
                            who.addItemToInventoryBool(__instance.objects[vector].getOne());
                            who.gainExperience(2, 7);
                        }

                        __instance.objects.Remove(vector);

                        __result = true;
                        return false;
                    }

                    __instance.objects[vector].Quality = quality;
                }
            }

            if (who.isRidingHorse())
            {
                who.mount.checkAction(who, __instance);

                __result = true;
                return false;
            }

            foreach (MapSeat current4 in __instance.mapSeats)
            {
                if (current4.OccupiesTile(tileLocation.X, tileLocation.Y) && !current4.IsBlocked(__instance))
                {
            ...
            foreach (KeyValuePair<Vector2, TerrainFeature> current5 in __instance.terrainFeatures.Pairs)
            {
                if (current5.Value.getBoundingBox(current5.Key).Intersects(value) && current5.Value.performUseAction(current5.Key, __instance))
                {
                    Game1.haltAfterCheck = false;
            ...
            {
                foreach (LargeTerrainFeature current6 in __instance.largeTerrainFeatures)
                {
                    if (current6.getBoundingBox().Intersects(value) && current6.performUseAction(current6.tilePosition, __instance))
                    {
                        Game1.haltAfterCheck = false;
            ...
            if (text == null)
            {
                text = __instance.doesTileHaveProperty(tileLocation.X, tileLocation.Y, "Action", "Buildings");
            }
            ...
            if (text != null)
            {
                if (__instance.currentEvent == null
                    ...
                    && !nPC.IsInvisible
                    && !nPC.IsMonster
                    && (!who.isRidingHorse() || !(nPC is Horse))
                    && Utility.withinRadiusOfPlayer(nPC.getStandingX(), nPC.getStandingY(), 1, who)
                    && nPC.checkAction(who, __instance))
                {
                ...
                __result = __instance.performAction(text, who, tileLocation);
                return false;
            }

            if (tile != null && __instance.checkTileIndexAction(tile.TileIndex))
            {
                __result = true;
                return false;
            }
            ...
            foreach (Furniture current7 in __instance.furniture)
            {
                if (current7.boundingBox.Value.Contains((int)(vector.X * 64f), (int)(vector.Y * 64f)) && (int)current7.furniture_type != 12)
                {
                    if (flag3)
                    {
                        ...
                        __result = current7.checkForAction(who);
                        return false;
                    }

                    __result = current7.clicked(who);
                    return false;
                }
        ...
        private static bool BeforePressActionButton(KeyboardState currentKBState, MouseState currentMouseState, GamePadState currentPadState, ref bool __result)
        {
            if (Game1.IsChatting)
            {
            ...
            if (Game1.dialogueUp && Game1.numberOfSelectedItems == -1)
            {
                ...
                else
                {
                    ...
                    ClickToMoveManager.Reflection.GetMethod(typeof(Game1), "checkIfDialogueIsQuestion").Invoke();
                }
                ...
                if (Game1.questOfTheDay != null && Game1.questOfTheDay.accepted.Value && Game1.questOfTheDay is SocializeQuest quest)
                {
                    quest.checkIfComplete(null, -1, -1);
                }
        ...
        private static bool BeforeTryToCheckAt(Vector2 grabTile, Farmer who, ref bool __result)
        {
        ...
        {
            // Reset the ClickToMove object associated with the current game location after the game
            // checks that the current location is not null.
